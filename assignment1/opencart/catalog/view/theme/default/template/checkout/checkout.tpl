...
<div id="content"><?php echo $content_top; ?>
  ...
  <div class="checkout">
    ...
    <div id ="referral">
    	<div class="checkout-heading"><?php echo $text_checkout_referral; ?></div>
    	<div class="checkout-content"></div>
    </div>
    <div id="confirm">
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				$.ajax({
					url: 'index.php?route=checkout/referral',
					dataType: 'html',
					success: function(html) {
						$('#referral .checkout-content').html(html);
						
						$('#payment-method .checkout-content').slideUp('slow');
						
						$('#referral .checkout-content').slideDown('slow');
...
// Referral button
$('#button-referral').live('click', function() {
	$.ajax({
		url: 'index.php?route=checkout/referral/validate',
		type: 'post',
		data: {
			referral_id: $('#referral input').val(),
		},
		dataType: 'json',
		beforeSend: function() {
			$('#button-referral').attr('disabled', true);
			$('#button-referral').after('<span class="wait">&nbsp;<img src="catalog/view/theme/default/image/loading.gif" alt="" /></span>');
		},
		complete: function() {
			$('#button-referral').attr('disabled', false);
			$('.wait').remove();
		},
		success: function(json) {
			$('.warning, .error').remove();
			if (json['error']) {
				if (json['error']['warning']) {
					$('#referral .checkout-content').prepend('<div class="warning" style="display: none;">' + json['error']['warning'] + '<img src="catalog/view/theme/default/image/close.png" alt="" class="close" /></div>');
					$('.warning').fadeIn('slow');
				}
			} else {
				$.ajax({
					...
					success: function(html) {
						...
						$('#referral .checkout-content').slideUp('slow');
						$('#confirm .checkout-content').slideDown('slow');
						$('#referral .checkout-heading a').remove();
						$('#referral .checkout-heading').append('<a><?php echo $text_modify; ?></a>');
					},
					error: function(xhr, ajaxOptions, thrownError) {
						alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
					}
