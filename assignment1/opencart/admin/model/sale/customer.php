...
class ModelSaleCustomer extends Model {
	...
	public function getBestReferrerId() {
		$query = $this->db->query("SELECT *, SUM(total) as sum FROM (SELECT * FROM " . DB_PREFIX . "order WHERE referral_id IS NOT NULL AND referral_id != 0) as o GROUP BY referral_id ORDER BY sum DESC LIMIT 1");

		return $query->row['referral_id'];
	}
